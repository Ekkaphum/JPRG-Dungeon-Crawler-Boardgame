...
describe('Set Trap legal slots (§v0.3.0.2)', () => {
  ...
  it('arms the trap when the slot is inside the window', () => {
    ...
    declareSkill(state, kit, { kind: 'DECLARE_ACTION', skillId: 'Trap', trapSlot: 18 }, createRNG(1));
  ...
  it('rejects a slot outside the window instead of silently arming it', () => {
    ...
    expect(() => declareSkill(state, kit, { kind: 'DECLARE_ACTION', skillId: 'Trap', trapSlot: 3 }, createRNG(1))).toThrow(/illegal Trap slot/);
  ...
  it('rejects a declare with no slot at all', () => {
    ...
    expect(() => declareSkill(state, kit, { kind: 'DECLARE_ACTION', skillId: 'Trap' }, createRNG(1))).toThrow(/illegal Trap slot/);
    expect(state.battle!.traps).toHaveLength(0);
  });
